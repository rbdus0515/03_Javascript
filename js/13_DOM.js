...
document.getElementById("btn1").addEventListener("click", function() {
    ...
    // Node 탐색
    // 1. 부모 Node 탐색 : parentNode
    const li1 = document.getElementById("li1");
    console.log( li1.parentNode ); // ul태그부터 나옴

    // 부모 Node 마지막에 새로운 Node 추가 (append : (마지막에) 덧붙이다.)
    li1.parentNode.append("abcd");

    // 2. 첫번째 자식 Node 탐색 : firstChild
    console.log( document.getElementById("test").firstChild );

    // 3. 마지막 자식 Node 탐색 : lastChild
    console.log( document.getElementById("test").lastChild ); // 추가된 abcd

    // 4. 중간에 존재하는 자식 Node 탐색 : 부모요소.childNodes[인덱스]
    console.log(nodeList[11]);

    // 5. 이전 형제 Node 탐색 : previousSibling
    //    다음 형제 Node 탐색 : nextSibling

    console.log(nodeList[8].previousSibling);
    console.log(nodeList[8].nextSiblingng);
...
document.getElementById("btn2").addEventListener("click", function() {
    ...
    // #test의 자식중 2번 인덱스의 이전 형제 요소 배경색 변경
